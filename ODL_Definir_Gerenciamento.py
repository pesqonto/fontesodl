...
# Escreve os dados de governança no formato xml
def gerar_xml_gerenciamento():
    ...
    pre_xml = pd.DataFrame.from_dict(
        {'Enlace': [def_servico[0]],
         ...
         'Gerenciamento_Banda_Desejavel': [info_banda_desejavel],
         'Gerenciamento_Banda_Limitrofe': [info_banda_minima],
         'Gerenciamento_SLA_Desejavel': [info_sla_razoavel],
         'Gerenciamento_SLA_Limitrofe': [info_sla_maximo],
         'Gerenciamento_Custo_Desejavel': [valores['custo_desejavel']],
         'Gerenciamento_Custo_Limitrofe': [valores['custo_minimo']]})

    nome_arquivo = 'ODL_Parametros_Gerenciamento_' + def_servico[0].replace(' ', '').upper() + '.xml'
...
def validar_entrada():
    valido = True
    if (not valores['banda_desejavel_100mbps'] and not valores['banda_desejavel_250mbps'] and not valores[
        'banda_desejavel_500mbps'] and not valores['banda_desejavel_1gbps'] and not valores['banda_desejavel_10gbps']):
        valido = False
    if (not valores['banda_minima_100mbps'] and not valores['banda_minima_250mbps'] and not valores[
        'banda_minima_500mbps'] and not valores['banda_minima_1gbps'] and not valores['banda_minima_10gbps']):
        valido = False
    ...
    if (not valores['sla_maximo_3'] and not valores['sla_maximo_6'] and not valores['sla_maximo_12'] and not valores['sla_maximo_24'] and not valores['sla_maximo_48']):
        valido = False
    if (valores['custo_desejavel'] == "") or (valores['custo_minimo'] == ""):
        valido = False
...
coluna = [
    [gui.Text('FASE 1 - ENTRADA DE DADOS - PROCESSO "DEFINIR PARÂMETROS DO NÍVEL DE GERENCIAMENTO"', font='Bold')],
    [gui.Text(' ')],
    ...
    [gui.Text(' ')],
    [gui.Text(
        'Considerando a futura EFETIVIDADE desse enlace, defina os valores DESEJÁVEIS e LIMÍTROFES para os seguintes parâmetros:',
        font='Bold')],
    [gui.Text(' ')],
    ...
    [gui.Text('(3) SLA Razoável', size=(15, 0)),
    ...
    [gui.Text('(4) SLA Máximo', size=(15, 0)),
    ...
    [gui.Text('(5) Custo Mínimo (R$/Mbps)', size=(25, 0)),
     gui.InputText(key='custo_minimo', size=(10, 0))],
    [gui.Text('(6) Custo Desejável (R$/Mbps)', size=(25, 0)),
     gui.InputText(key='custo_desejavel', size=(10, 0))],
    [gui.Text(' ')],
...
#Exibe tela
janela = gui.Window('Projeto OntoDataLink',layout, resizable=True, size=(1400,1000) )
...
while True:
    ...
    if (evento == 'Ajuda'):
        messagebox.showinfo('Orientações para a definição dos parâmetros', \
                            '(1) Banda mínima - Informe o “limite mínimo” de largura de banda para suportar os serviços; abaixo desse valor, entende-se que haverá comprometimento da entrega dos serviços listados no Catálogo se Serviços. Defina o valor de acordo com a perspectiva de entrega segundo o Catálogo de Serviços. (Polaridade – valor maior é melhor).\n\n' + \
                            '(2) Banda desejável - Informe o valor confortável de largura de banda para suportar os serviços. Defina o valor de acordo com a perspectiva de entrega segundo o Catálogo de Serviços. (Polaridade – valor maior é melhor).\n\n' + \
                            '(3) SLA Razoável - Informe o prazo razoável para restabelecer os serviços em caso de falha no enlace. Defina o valor de acordo com a perspectiva do Catálogo de Serviços. Polaridade – valor menor é melhor).\n\n' + \
                            '(4) SLA Máximo - Informe o “limite máximo” de tempo para restabelecer a conectividade em caso de falha no enlace. Acima desse valor, entende-se que haverá comprometimento na entrega dos serviços listados no Catálogo se Serviços. (Polaridade – valor menor é melhor).\n\n' + \
                            '(5) Custo Mínimo (R$/Mbps) - Informe o “limite mínimo” necessário para um processo viável de contratação do enlace no mercado local de TI. Abaixo dese valor, entende-se que será provável que não haverá fornecedor interessado na contratação. Defina o valor de acordo com a realidade do mercado local de serviços de TI. (Polaridade – valor maior é melhor).\n\n' + \
                            '(6) Custo Desejável (R$/Mbps) - Informe o valor financeiro confortável para viabilizar a contratação do enlace (informe um valor inteiro). Defina o valor de acordo com a realidade do mercado local de serviços de TI. (Polaridade – valor maior é melhor).')

    if (evento == 'Salvar'):
        if validar_entrada():
            gerar_xml_gerenciamento()
            break
